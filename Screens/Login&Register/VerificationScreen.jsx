...
export default function VerificationScreen({ navigation, route, props }) {
    ...
    const confirmCode = async () => {
        ...
        try {
            ...
            await axios.post(`http://172.16.4.69:5000/user/signup`, userData).then(async (res) => {
                if (res.status === 200) {
                    console.log("User created successfully");
                    await saveDetails(res.data.data._id);
                }
            })
            await axios.post("http://172.16.4.69:5000/user/login", {email: email, password: storeData.data.password}).then((res) => {
    ...
    const fetchTempData = async () => {
        try {
            await axios.post(`http://172.16.4.69:5000/user/getTempDetails`, {email : email}).then((res) => {
    ...
    const verifyChangedNumber = async () => {
        try {
            await axios.post(`http://172.16.4.69:5000/user/getUserDetailsByMobile`, {mobile : mobile}).then(async (res) => {
                if (res.status === 200) {
                    setModalVisible(false);
            ...
            await axios.patch(`http://172.16.4.69:5000/user/updateTempNumber`, {email : email, mobile : mobile}).then((res) => {
                    if (res.status === 200) {
