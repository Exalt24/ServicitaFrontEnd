...
export default function LoginPage ({ navigation }) {
            ...
            const infoRequest = new GraphRequest('/me?fields=email,first_name,last_name', null, async (error, result) => {
              if (error) {
                  ...
                  .then((emailExists) => {
                    if (emailExists.exists) {
                        ...
                        if (emailExists.type === "permanent") {
                          axios.get(`http://192.168.1.9:5000/admin/checkSuspensionStatus/${userEmail}`).then((res) => {
                            if (res.data.status === 'SUCCESS') {

                              if (res.data.type === 'SUSPENDED') {
                                const remainingTime = res.data.remainingTime;
                                if (remainingTime <= 0) {
                                  axios.patch(`http://192.168.1.9:5000/admin/unsuspendUser`, { email: userData.email }).then((res) => {
                                    if (res.data.status === 'SUCCESS') {
                                      axios.post("http://192.168.1.9:5000/user/loginOther", {email: userData.email }).then((res) => {
                ...
                Alert.alert('Account Suspended', `Your account has been suspended. You will be able to login again in ${formatMessage}.`, [{ text: 'OK' }]);
                                ...
                                axios.post("http://192.168.1.9:5000/user/loginOther", userEmail).then((res) => {
                                    console.log(res.data)
                                    if (res.data.status === 'SUCCESS') {
                                        Alert.alert('Success', 'You have successfully logged in.', [{ text: 'OK' }]);
                                        AsyncStorage.setItem('token', res.data.data);
    ...
    const GoogleSignIn = async () => {
        try {
          ...
          if (emailExists.exists) {
            GoogleLogOut();
            if (emailExists.type === "permanent") {
              axios.get(`http://192.168.1.9:5000/admin/checkSuspensionStatus/${userData.email}`).then((res) => {
                if (res.data.status === 'SUCCESS') {
                  if (res.data.type === 'SUSPENDED') {
                    const remainingTime = res.data.remainingTime;
                    if (remainingTime <= 0) {
                      axios.patch(`http://192.168.1.9:5000/admin/unsuspendUser`, { email: userData.email }).then((res) => {
                        if (res.data.status === 'SUCCESS') {
                          axios.post("http://192.168.1.9:5000/user/loginOther", {email: userInfo.user.email }).then((res) => {
                            console.log(res.data)
                            if (res.data.status === 'SUCCESS') {
                                Alert.alert('Success', 'You have successfully logged in.', [{ text: 'OK' }]);
                                AsyncStorage.setItem('token', res.data.data);
    ...
    const handleSubmit = () => {
        ...
        axios.post("http://192.168.1.9:5000/user/login", userData).then((res) => {
        const storedData = res.data.data;
        if (res.data.status === 'SUCCESS') {
          axios.get(`http://192.168.1.9:5000/admin/checkSuspensionStatus/${email}`).then((res) => {
            if (res.data.status === 'SUCCESS') {
              if (res.data.type === 'SUSPENDED') {
                const remainingTime = res.data.remainingTime;
                if (remainingTime <= 0) {
